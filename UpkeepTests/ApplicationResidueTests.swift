...
final class ApplicationResidueTests: XCTestCase {
  ...
  func testDoesNotListProtectedRecentDocuments() throws {
    ...
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepResidueSFL-\(UUID().uuidString)", isDirectory: true)
  ...
  func testScannerFindsFixtureFilesAndSkipsOtherTeamApps() throws {
    let fileManager = FileManager.default
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepResidue-\(UUID().uuidString)", isDirectory: true)
    defer { try? fileManager.removeItem(at: fixture) }
  ...
  func testScannerSkipsSystemManagedDarwinRuntimeDirectories() throws {
    let fileManager = FileManager.default
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepResidueManagedDarwin-\(UUID().uuidString)", isDirectory: true)
    defer { try? fileManager.removeItem(at: fixture) }
  ...
  func testScannerIncludesCaskroomForHomebrewManagedSparkleApplication() throws {
    let fileManager = FileManager.default
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepResidueCaskroom-\(UUID().uuidString)", isDirectory: true)
    defer { try? fileManager.removeItem(at: fixture) }
  ...
  func testUninstallerMovesSelectedFilesToTrash() async throws {
    ...
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepUninstall-\(UUID().uuidString)", isDirectory: true)
  ...
  func testUninstallerUsesHomebrewTokenForSparkleApplicationInstalledByCask() async throws {
    let fileManager = FileManager.default
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepBrewUninstall-\(UUID().uuidString)", isDirectory: true)
  ...
  func testUninstallerUsesFinderFallbackAndVerifiesEveryPathWasRemoved() async throws {
    let fileManager = FileManager.default
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepFinderTrash-\(UUID().uuidString)", isDirectory: true)
    let applicationURL = directory.appendingPathComponent("Demo.app", isDirectory: true)
    let residueURL = directory.appendingPathComponent("com.example.demo.plist")
    defer { try? fileManager.removeItem(at: directory) }
  ...
  func testUninstallerQuitsRunningApplicationBeforeRemovingResidueOnly() async throws {
    let fileManager = FileManager.default
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepResidueOnlyQuit-\(UUID().uuidString)", isDirectory: true)
    let applicationURL = directory.appendingPathComponent("Demo.app", isDirectory: true)
    let residueURL = directory.appendingPathComponent("com.example.demo.plist")
    defer { try? fileManager.removeItem(at: directory) }
  ...
  func testUninstallerReportsFinderItemsThatStillExist() async throws {
    let fileManager = FileManager.default
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepFinderVerify-\(UUID().uuidString)", isDirectory: true)
    let applicationURL = directory.appendingPathComponent("Demo.app", isDirectory: true)
    defer { try? fileManager.removeItem(at: directory) }
    try fileManager.createDirectory(at: applicationURL, withIntermediateDirectories: true)
  ...
  func testUninstallerDoesNotIgnoreAFallbackCompletionError() async throws {
    let fileManager = FileManager.default
    let directory = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepTrashCompletion-\(UUID().uuidString)", isDirectory: true)
    let applicationURL = directory.appendingPathComponent("Demo.app", isDirectory: true)
    defer { try? fileManager.removeItem(at: directory) }
    try fileManager.createDirectory(at: applicationURL, withIntermediateDirectories: true)
  ...
  func testDarwinVolatileTargetsMoveIntoUserTrashDirectly() async throws {
    let fileManager = FileManager.default
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepUserTrashMove-\(UUID().uuidString)", isDirectory: true)
    defer { try? fileManager.removeItem(at: fixture) }
  ...
  func testUserTrashMoveRejectsNonVolatileTargets() async throws {
    let fileManager = FileManager.default
    let fixture = fileManager.temporaryDirectory
      .appendingPathComponent("UpkeepUserTrashReject-\(UUID().uuidString)", isDirectory: true)
    defer { try? fileManager.removeItem(at: fixture) }
